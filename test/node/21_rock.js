...
import { Window, Color, Camera, Vec2, Vec3, Vec, loop } from "../../src/index.node.js";
// resize incoming canvas:Canvas object.
const width = 640 / 3;
const height = 480 / 3;

const window = Window.ofSize(width, height);
window.setWindowSize(width * 3, height * 3);
...
loop(async ({ dt, time }) => {
    ...
    (
        await camera
            ...
            .to(window, { lightPosSerial: light.pos.toArray(), hyperPlanesSerial: hyperPlanes })
    ).paint();
    window.setTitle(`Inigo Iq - Tela Rock | FPS: ${Math.floor(1 / dt)}`);
}).play();