async (canvas, logger) => {
    ...
    function enforceConstraints(path, { otherPaths, shape }, dt) {
        const n = path.length;
        if (dt === 0) return;
        ...
        let sumCos = 0;
        for (let i = 0; i < n; i++) {
            sumSin += shape[i].cross(centeredPath[i]);
            sumCos += shape[i].dot(centeredPath[i]);
        }
    ...
    function updateScene(dt) {
        ...
        for (let i = 0; i < n; i++) {
            ...
            for (let k = 0; k < subSteps; k++) {
                ...
                enforceConstraints(
                    path,
                    { otherPaths: paths.filter((_, l) => i !== l), shape },
                    delta
                );
