...
async (canvas, logger) => {
    ...
    function smin(a, b, k = 50) {
        const res = Math.exp(-k * a) + Math.exp(-k * b);
        return -Math.log(res) / k;
    }

    function render(ray, { scene }) {
        if (!scene) return Color.BLACK;
    ...
    // scene
    loop(async ({ dt, time }) => {
        (await camera.rayMapParallel(render, [smin]).to(canvas, { scene })).paint();
        logger.print(`${(1 / dt).toFixed(2)}`);
